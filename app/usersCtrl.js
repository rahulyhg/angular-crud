app.controller('usersCtrl', function ($scope, $modal, $filter, Data) {
    ...
    $scope.changeUserStatus = function(user){
        ...
        Data.post("users/update/"+user.id,{status:user.status});
    };
    ...
    $scope.open = function (p,size) {
        ...
        modalInstance.result.then(function(selectedObject) {
            if(selectedObject.save == "insert"){
                ...
                $scope.users = $filter('orderBy')($scope.users, 'id', 'reverse');
...
app.controller('userEditCtrl', function ($scope, $modalInstance, item, Data) {
  ...
  $scope.user = angular.copy(item);
        ...
        $scope.title = (item.id > 0) ? 'Edit User' : 'Add User';
        ...
        $scope.saveUser = function (user) {
            user.uid = $scope.uid;
            if(user.id > 0){
                Data.put('users/'+user.id, user).then(function (result) {
